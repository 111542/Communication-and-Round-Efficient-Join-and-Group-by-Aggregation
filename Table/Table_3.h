...
/*void aggre_sum(int role, uint32_t bitlen, vector<uint32_t>& Ind, STable& T, STable& R, int m, crypto* crypt){
    ...
    if(role != 0){
       ...
       ABYParty* pa = new ABYParty((e_role)(role-1), address, 7766, seclvl, bitlen, nthreads, mt_alg);
       vector<Sharing*>& sharings_a2b = pa->GetSharings();
       Circuit* bct = sharings_a2b[S_BOOL]->GetCircuitBuildRoutine();
       Circuit* act = sharings_a2b[S_ARITH]->GetCircuitBuildRoutine();
       Circuit* yct = sharings_a2b[S_YAO]->GetCircuitBuildRoutine();
        ...
        share* val_a2b = act->PutSharedSIMDINGate(R.len, R.s_S.s2x, bitlen);
        share* rl_a2b = bct->PutSharedSIMDINGate(R.len, r1, bitlen);
        val_a2b = bct->PutA2BGate(val_a2b, yct);
        val_a2b = bct->PutXORGate(val_a2b, rl_a2b);
        //val = circ->PutA2BGate(val, yc);
        share* s_out1 = bct->PutOUTGate(val_a2b, ALL);
        cout<<"222222222222"<<endl;
        pa->ExecCircuit();
        cout<<"2    end"<<endl;
        std::cout << pa->GetTiming(P_SETUP) << "\t" << pa->GetTiming(P_ONLINE) << "\t" << pa->GetTiming(P_TOTAL) << std::endl;
