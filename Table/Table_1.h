...
//p1, p2改成a，b向量的pair，直接一起算会快一点pair<vector<uint64_t>, vector<uint64_t>>& res
/*void mpc_agg(int role, int operation, int bitlen, pair<share*, share*>& p1, pair<share*, share*>& p2, pair<share*, share*>& res, BooleanCircuit* circ){
    
    

    share* s_a1, *s_b1, *s_a2, *s_b2, *s_a, *s_b, *s_tmp, *s_sel;
    s_a1 = p1.first;
    s_b1 = p1.second;
    s_a2 = p2.first;
    s_b2 = p2.second;
...
//ac->PutPrintValueGate(s_b2, "input_b2");
    ...
    s_a = circ->PutMULGate(s_a1, s_a2);

    s_tmp = circ->PutMULGate(s_a2, s_b1);
...
//A转B吧，没有A的GTGate
    ...
    s_sel = circ->PutGTGate(s_tmp, s_b2);
    
    if(operation == 1){
        s_b = circ->PutMUXGate(s_tmp, s_b2, s_sel);
    }else{
        s_b = circ->PutMUXGate(s_b2, s_tmp, s_sel);
    }
 //bc->PutPrintValueGate(s_b, "mggb");
    ...
    res = make_pair(s_a, s_b);
   // ac->PutPrintValueGate(s_a, "a");
...
/*void mpc_agg(int role, int operation, uint32_t bitlen, pair<Data, Data>& p1, pair<Data, Data>& p2, pair<Data, Data>& res, crypto* crypt){
    ...
    share* s_a1, *s_b1, *s_a2, *s_b2, *s_a, *s_b, *s_tmp, *s_sel;
    s_a1 = bc->PutSharedINGate(p1.first, bitlen);

    s_b1 = bc->PutSharedINGate(p1.second, bitlen);

    s_a2 = bc->PutSharedINGate(p2.first, bitlen);
    s_b2 = bc->PutSharedINGate(p2.second, bitlen);
...
//A转B吧，没有A的GTGate
 ...
 //bc->PutPrintValueGate(s_b, "mggb");
    ...
    Data r;
    randombytes_buf(&r, 0);
    share* s_r = bc->PutSharedINGate(r, bitlen);
    ...
    s_b->get_clear_value_vec(&res_b, &bitlen, &nvals);
    ...
    res = make_pair(r_a.s2x[0], r_b.s2x[0]);
   // ac->PutPrintValueGate(s_a, "a");
...
void mpc_agg(int role, int operation, uint32_t bitlen, Data* a1, Data* b1, Data* a2, Data* b2, Data** Ma, Data** Mb, int rows, int j, crypto* crypt){
    ...
    share* s_a1, *s_b1, *s_a2, *s_b2, *s_a, *s_b, *s_tmp, *s_sel;
    s_a1 = bc->PutSharedSIMDINGate(rows, &a1[1], bitlen);
...
//
uint8_t** F_encode(int role, uint32_t bitlen, CBitVector& key, LowMCParams* param , crypto* crypt, boolSh2& s){
//cout<<"111"<<endl;
...
bool compare(uint8_t** Ex, uint8_t** Ey, int inx, int iny){
...
//operation = 1: semi-join， 
    ...
    if(role == 0){
        ...
        Ex = F_encode(role, bitlen, key, &param, crypt, X.s_K);   //return Ex
        //gettimeofday(&exend, 0);
        //cout<<"ex time: "<<exend.tv_sec-exbegin.tv_sec<<endl;
        Ey = F_encode(role, bitlen, key, &param, crypt, Y.s_K);
        //gettimeofday(&eyend, 0);
        //cout<<"ey time: "<<eyend.tv_sec-exend.tv_sec<<endl;
    ...
    //！！！！！！！就是这！！！测试之后去掉            //break;
       ...
       // gettimeofday(&shufflebegin, 0);
        ...
        //cout<<"shuffle 1 time: "<<shuffleoneend.tv_sec-shufflebegin.tv_sec<<endl;
        //cout<<"shuffle all time: "<<shuffleend.tv_sec-shufflebegin.tv_sec<<endl;
        ...
        //P1，P2        
        F_encode(role, bitlen, key, &param, crypt, X.s_K);
        
        F_encode(role, bitlen, key, &param, crypt, Y.s_K);
...
uint32_t group_by(int role, int bitlen,vector<uint32_t>& Ind, LowMCParams* param, crypto* crypt, STable& T, STable& R){   //operation是aggregation的函数
    ...
    uint32_t m = 1;
    uint8_t** ET;
    ET = F_encode(role, bitlen, key, param, crypt, T.s_K);
...
/*boolSh2& aggre_m(int role, int operation, vector<uint32_t>& Ind, boolSh2& Rs, boolSh2& O, int m, crypto* crypt){
//operation = 1: max
    
    
    boolSh2 sInd(Ind.size());
    O.init(m);
    int n = Rs.len;
...
//P1,P2
    if(role != 0){
        ...
        if(role == 1){
            ...
            boolRecvFromParty(sInd.s2x, sInd.len, (role+2)%3, XSHARETAG);
            ...
            //cout<<"share end"<<endl;
            //cout<<"P2 1111111111111111111111111"<<endl;
    ...
    //从这里的a1开始都变share类型吧
            ...
            a1 = bc->PutSharedINGate(sInd.s2x[2*i-2], bitlen);
            b1 = bc->PutSharedINGate(Rs.s2x[2*i-2], bitlen);
            auto p1 = make_pair(a1, b1);
    //cout<<"p1";
            a2 = bc->PutSharedINGate(sInd.s2x[2*i-1], bitlen);
            b2 = bc->PutSharedINGate(Rs.s2x[2*i-1], bitlen);
            auto p2 = make_pair(a2, b2);
            //pair(<vector<uint32_t>, vector<uint32_t>) res;
    //cout<<",p2"<<endl;
            pair<share*, share*> res;

            /*if(i == 2){
                circ->PutPrintValueGate(b1, "b1");
                circ->PutPrintValueGate(b2, "b2");

            }*/
    //
            //circ->PutPrintValueGate(a1, "a1");
                //circ->PutPrintValueGate(a2, "a2");

  /*          mpc_agg(role, operation, bitlen, p1, p2, res, (BooleanCircuit*)bc);

            Ma[i][1] = res.first;
            Mb[i][1] = res.second;
            //circ->PutPrintValueGate(Ma[i][1], "ma");
            //circ->PutPrintValueGate(Mb[i][1], "mb");
            if(role == 1){
                //cout<<"i: "<<i<<endl;
            }
		...
		b1 = Mb[(2*k+1)*t][j-1];
               ...
               // a1 = Ma[(2*k+1)*pow(2, j-2)][j-1];
               // b1 = Mb[(2*k+1)*pow(2, j-2)][j-1];
                auto p1 = make_pair(a1, b1);
                ...
                if(q == 0){
                   ...
                   if(ind>n){
                    ...
                    a2 = Ma[inda][indb];
		    b2 = Mb[inda][indb];
                   // a2 = Ma[(k+1)*pow(2, j-1) - pow(2, j-2) + q+1 - pow(2, ceil(log2(q+1))-1)][ceil(log2(q+1))];
                   // b2 = Mb[(k+1)*pow(2, j-1) - pow(2, j-2) + q+1 - pow(2, ceil(log2(q+1))-1)][ceil(log2(q+1))];
               ...
               // auto tmp = mpc_agg(role, operation, p1, p2, circ);
                //mpc_agg(role, operation, bitlen, p1, p2, res,(ArithmeticCircuit*)circ);
                //Ma[i][j] = tmp.first;
               // Mb[i][j] = tmp.second;
               pair<share*, share*> res;
                ...
                Mb[i][j] = res.second;
                //cout<<"i: "<<i<<", j: "<<j<<endl;
                //circ->PutPrintValueGate(Ma[i][j], "ma");
                //circ->PutPrintValueGate(Mb[i][j], "mb");
                //cout<<endl;
            }        
        }
        ...
        vector<share*> sst(L.len);

        //cout<<"sst.len: "<<sst.size()<<endl;
        //L.s2x[0] = Rs.s2x[0];
        //sst[0] = circ->PutSharedINGate(Rs.s2x[0], bitlen);
        /*for(int i=1; i<=rows; i++){
            for(int j=1; j<=cols; j++){
                
                circ->PutPrintValueGate(Ma[i][j], "a");
                circ->PutPrintValueGate(Mb[i][j], "b");
            }
        }*/
        
         //cout<<"sst.size(): "<<sst.size()<<endl;
        /*for(int i=2; i<sst.size(); i++){
            sst[i] = circ->PutSharedOUTGate(Mb[i-pow(2, floor(log2(i-1)))][ceil(log2(i))]);
            circ->PutPrintValueGate(sst[i], "sst");
        }*/
   /*     sst[0] = bc->PutSharedINGate(Rs.s2x[0], bitlen);
        //circ->PutPrintValueGate(sst[0], "sst");
        for(int i=2; i<=sst.size(); i++){
            sst[i-1] = Mb[static_cast<int>(i-pow(2, floor(log2(i-1))))][static_cast<int>(ceil(log2(i)))];
            //circ->PutPrintValueGate(sst[i-1], "sst");
        }

        //cout<<"P"<<role<<" end"<<endl;
        //cout<<"11111111111111111111111111111111111111111111111111"<<endl;

        
        vector<share*> sRran(sst.size());
        Data Rran[sst.size()];
       
       //
       //cout<<"kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk"<<endl;
        for(int i=0; i<sRran.size(); i++){
                
                //Rran[i] = rand()%range+1;
                randombytes_buf(&Rran[i], sizeof(Data));
                //cout<<"i: "<<i<<" ran: "<<Rran[i]<<endl;
                sRran[i] = bc->PutSharedINGate(Rran[i], bitlen);
                //circ->PutPrintValueGate(sRran[i], "sRran");
                sst[i] = bc->PutXORGate(sst[i], sRran[i]);
                //circ->PutPrintValueGate(sst[i], "en_sst");
                sst[i] = bc->PutOUTGate(sst[i], SERVER);
        }
        ...
        party->ExecCircuit();
        vector<uint64_t> en_Rm(sst.size());
        if(role == 1){
            //cout<<"share:================================================="<<endl;
            for(int i=0; i<en_Rm.size(); i++){
                uint8_t* out = new uint8_t[bitlen/8];
                out = sst[i]->get_clear_value_ptr();
                en_Rm[i] = 0;
                for(int j=0; j<bitlen/8; j++){
                    en_Rm[i] |= (out[j]<<j*8);
                    //cout<<"out: "<<(int)out[j]<<endl;
                }
                //cout<<en_Rm[i]<<endl;

            }
            L.share(role, 2, 1, en_Rm.data());

            
        }else{
            L.share(role, 1, 1, en_Rm.data());
        }
...
//P1,P2
    ...
    /*        mpc_agg(role, operation, bitlen, p1, p2, res, crypt);
...
//P1,P2
 ...
 /*           mpc_agg(role, operation, bitlen, p1, p2, res, (BooleanCircuit*)bc);
  ...
  /*      sst[0] = bc->PutSharedINGate(Rs.s2x[0], bitlen);
        //circ->PutPrintValueGate(sst[0], "sst");
        for(int i=2; i<=sst.size(); i++){
            sst[i-1] = Mb[i-pow(2, floor(log2(i-1)))][ceil(log2(i))];
            //circ->PutPrintValueGate(sst[i-1], "sst");
        }
       ...
       //cout<<"kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk"<<endl;
        ...
        party->ExecCircuit();
        vector<uint32_t> en_Rm(sst.size());
        if(role == 1){
            //cout<<"share:================================================="<<endl;
            for(int i=0; i<en_Rm.size(); i++){
                uint8_t* out = new uint8_t[bitlen/8];
...
//P1,P2
    ...
    //
        ...
        mpc_agg(role, operation, bitlen, a1, b1, a2, b2, Ma, Mb, rows, 1, crypt);
        cout<<"P"<<role<<"第一列结束"<<rows<<" "<<cols<<endl;
        //cout<<"111"<<endl;
        //cout<<"p12==========================================="<<endl;
        //验这里dddd
        ...
        for(int j=2; j<=cols; j++){
            ...
            mpc_agg(role, operation, bitlen, a1, b1, a2, b2, Ma, Mb, rows, j, crypt);        
        }
